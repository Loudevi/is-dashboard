...
@Component({
    selector: '[e-sale-template]',
    styleUrls: [ './e-sale-template.style.scss' ],
    templateUrl: './e-sale-template.template.html'
})

export class ESaleTemplateComponent implements OnInit {
    ...
    newDocument() {
        ...
        this._completeDocument.insertElectronic(this.completeDocument)
            .subscribe(res => {
                this.spinner = false;
    ...
    addProductDetail() {
        if ( this.product.productTypeId == 1 ) {
            if ( this.stock < this.detail.qty ) {
                this._toastr.warning('No hay stock suficiente del articulo que desea vender!', 'Stock Insuficiente', { timeOut: 5000 });
            } else {
                this.detail.id = 0;
    ...
    stakeholderChange() {
        this._stakeholder.get(0, this.stakeholder.idDocument, 'S')
            .subscribe(res => {
    ...
    priceCalculation() {
        if(this.product.customPrice == 'T') {
            ...
            if(this.product.taxType == 'T') {
                ...
                this.detail.priceBeforeTaxes = this.product.priceBeforeTaxes;
                this.detail.tax = this.product.tax;
            } else {
                this.detail.discount = this.product.price - this.detail.price;
                this.detail.priceBeforeTaxes = this.detail.price;
                this.detail.tax = 0;
    ...
    calculateDocumentTotals(detail) {
        if(this.documentTotal.totalPrice == 0) {
            ...
            this.documentTotal.discount = (parseFloat(detail.discount) * parseFloat(detail.qty)).toFixed(2);
            ...
            this.documentTotal.discount = (parseFloat(this.documentTotal.discount) + (parseFloat(detail.discount) * parseFloat(detail.qty))).toFixed(2);
